...
or use [`jsQR.es.js`](./doc/jsQR.es.js) can be included with a script tag as ES module.

```html
<script src="jsQR.es.js" type="module"></script>
<script type="module">
	import {jsQR} from "./doc/jsQR.es.js";
	(async ()=>{ await jsQR(...); })()
</script>
```

or [`jsQR.es.min.js`](./doc/jsQR.es.min.js) which it is minify size.
or [`jsQR.es.min.nosjis.js`](./doc/jsQR.es.min.nosjis.js) which no use direct parsing kanji data to sjis encoded text.
...
access to http://localhost:8089/docs/test.html, run by load html file.
this test is conpare with original build jsQR with jsQR.es.
