...
class W {
	...
	static async getShiftJISTable() {
		return W.decompress(await W.decompress4DURI(W.SJIS_dURI));
	}
...
class DecodeData {
	...
	static decodeKanji(stream, size) {
		...
		for (let i = 0; i < length; i++) {
			...
			text += String.fromCharCode(W.shiftJISTable[c]);
		}
		return { bytes, text };
	}
	static decode(data, version) {
		...
		while (stream.available() >= 4) {
			...
			const Mode = DecodeData.Mode;
			const ModeByte = DecodeData.ModeByte;
...
class Decoder {
	...
	static readFormatInformation(matrix) {
		...
		let bestDifference = Infinity,
			bestFormatInfo = null;
		for (let _i = 0, FORMAT_INFO_TABLE_1 = Decoder.FORMAT_INFO_TABLE; _i < FORMAT_INFO_TABLE_1.length; _i++) {
	...
	static countBlackWhiteRunTowardsPoint(origin, end, matrix, length) {
		...
		for (let i = 0; i < length; i++)
			if (switchPoints[i] && switchPoints[i + 1])
				distances.push(Locator.distance(switchPoints[i], switchPoints[i + 1]));
