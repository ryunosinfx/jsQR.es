...
const VERSIONS = await W.getVERSIONS();
/* 8 */
class V {
	static compress(t) {
	...
	static async getShiftJISTable() {
		return V.decompress(await W.decompress4DURI(V.SJIS_dURI));
	}
}
const shiftJISTable = await V.getShiftJISTable();
/*0*/
class BitMatrix {
...
class Reedsolomon {
	...
	static decode(bytes, twoS) {
		...
		const sigmaOmega = Reedsolomon.runEuclideanAlgorithm(field, field.buildMonomial(twoS, 1), syndrome, twoS);
...
class Binarizer {
	...
	static binarize(data, width, height, returnInverted) {
		...
		const horizontalRegionCount = Math.ceil(width / Binarizer.REGION_SIZE);
		const verticalRegionCount = Math.ceil(height / Binarizer.REGION_SIZE);
	...
	static decodeAlphanumeric(stream, size) {
		...
		while (length >= 2) {
			const v = stream.readBits(11),
				...
				b = v % 45;
			bytes.push(
				DecodeData.AlphanumericCharacterCodes[a].charCodeAt(0),
				DecodeData.AlphanumericCharacterCodes[b].charCodeAt(0)
			);
			text += DecodeData.AlphanumericCharacterCodes[a] + DecodeData.AlphanumericCharacterCodes[b];
			length -= 2;
		}
		if (length === 1) {
			const a = stream.readBits(6);
			bytes.push(DecodeData.AlphanumericCharacterCodes[a].charCodeAt(0));
			text += DecodeData.AlphanumericCharacterCodes[a];
		}
		return { bytes, text };
	}
...
class Decoder {
	...
	static buildFunctionPatternMask(version) {
		const dimension = 17 + 4 * version.versionNumber,
			matrix = BitMatrix.createEmpty(dimension, dimension);
	...
	static readVersion(matrix) {
		...
		if (provisionalVersion <= 6) return VERSIONS[provisionalVersion - 1]; // 6 and under dont have version info in the QR code
	...
	static decodeMatrix(matrix) {
		const version = Decoder.readVersion(matrix);
		if (!version) return null;
		const formatInfo = Decoder.readFormatInformation(matrix);
		if (!formatInfo) return null;
		const codewords = Decoder.readCodewords(matrix, version, formatInfo),
			dataBlocks = Decoder.getDataBlocks(codewords, version, formatInfo.errorCorrectionLevel);
...
class Locator {
	...
	static countBlackWhiteRunTowardsPoint(origin, end, matrix, length) {
		...
		const distances = [];
		for (let i = 0; i < length; i++)
			if (switchPoints[i] && switchPoints[i + 1]) distances.push(distance(switchPoints[i], switchPoints[i + 1]));
	...
	static findAlignmentPattern(matrix, alignmentPatternQuads, topRight, topLeft, bottomLeft) {
		...
		try {
			const _a = Locator.computeDimension(topLeft, topRight, bottomLeft, matrix);
		...
		const bRFP = {
			...
			mBFP = (Locator.distance(topLeft, bottomLeft) + Locator.distance(topLeft, topRight)) / 2 / moduleSize, //modulesBetweenFinderPatterns
...
function scan(matrix) {
	...
	for (let _i = 0; _i < locations_1.length; _i++) {
		const location_1 = locations_1[_i],
			extracted = Extractor.extract(matrix, location_1),
			decoded = Decoder.decode(extracted.matrix);
		if (decoded)
			return {
...
export const jsQR = function (data, width, height, providedOptions = {}) {
	...
	const _a = Binarizer.binarize(data, width, height, shouldInvert),
