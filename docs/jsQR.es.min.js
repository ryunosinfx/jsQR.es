...
class W {
	...
	static ab2S = (a) => W.td.decode(W.u8a(a));
...
class LC {
	...
	static cB(o, e, m, l) {
		const r = e.y - o.y,
			u = e.x - o.x,
			t = LC.cBT(o, e, m, Math.ceil(l / 2)),
			a = LC.cBT(o, { x: o.x - u, y: o.y - r }, m, Math.ceil(l / 2)),
			v = t.shift() + a.shift() - 1;
		let i;
	...
	static sP(p, r, m) {
		try {
			const a = LC.cB(p, { x: -1, y: p.y }, m, r.length),
				b = LC.cB(p, { x: p.x, y: -1 }, m, r.length),
				M = Math,
				c = {
					x: M.max(0, p.x - p.y) - 1,
					y: M.max(0, p.y - p.x) - 1,
				},
				d = LC.cB(p, c, m, r.length),
				f = {
					x: M.min(m.w, p.x + p.y) + 1,
					y: M.min(m.h, p.y + p.x) + 1,
				},
				g = LC.cB(p, f, m, r.length),
				h = LC.sB(a, r),
				i = LC.sB(b, r),
				j = LC.sB(d, r),
				k = LC.sB(g, r),
	...
	static l(m) {
		...
		const G = P.filter((q) => q.B.y - q.T.y >= 2)
			.map((q) => {
				const x = (q.T.s + q.T.e + q.B.s + q.B.e) / 4,
					y = (q.T.y + q.B.y + 1) / 2;
				if (!m.g(M.round(x), M.round(y))) return;
				const l = [q.T.e - q.T.s, q.B.e - q.B.s, q.B.y - q.T.y + 1];
				return { C: LC.sP({ x: M.round(x), y: M.round(y) }, [1, 1, 3, 1, 1], m), x, y, S: LC.s(l) / l.length };
			})
			.filter((q) => !!q)
			.sort((a, b) => a.C - b.C)
			.map((t, i, f) => {
		...
		const i = LC.rFP(G[0].p[0], G[0].p[1], G[0].p[2]),
			H = i.tR,
			I = i.tL,
			J = i.bL,
			K = LC.fAP(m, Q, H, I, J),
	...
	static fAP(m, a, r, l, b) {
		let d, s;
		try {
			const i = LC.cD(l, r, b, m);
			d = i.dim;
			s = i.mS;
		} catch (e) {
			...
			h = a
				.map((q) => {
					...
					return !m.g(M.floor(x), M.floor(y))
						? 0
						: { x, y, C: LC.sP({ x: M.floor(x), y: M.floor(y) }, [1, 1, 1], m) + LC.d({ x, y }, g) };
				})
				.filter((v) => !!v)
				.sort((a, b) => a.C - b.C);
