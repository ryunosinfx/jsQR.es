...
class RS {
	...
	static fEL(f, eL) {
		...
		return eC !== nE ? W.n('RS fEL eC', eC, nE) : r;
	}
	static fEM(f, eE, eL) {
		const s = eL.length;
		const r = W.A(s);
...
class DC {
	...
	static v(m) {
		const d = m.h,
			a = Math.floor((d - 17) / 4);
		if (a <= 6) return W.VERSIONS[a - 1];
		let b = 0,
			c = 0;
		for (let y = 5; y >= 0; y--) for (let x = d - 9; x >= d - 11; x--) b = DC.p(m.get(x, y), b);
		for (let x = 5; x >= 0; x--) for (let y = d - 9; y >= d - 11; y--) c = DC.p(m.get(x, y), c);
		let e = Infinity,
			f = null;
	...
	static f(m) {
		let e = 0,
			f = 0;
		for (let x = 0; x <= 8; x++) if (x !== 6) e = DC.p(m.get(x, 8), e);
		for (let y = 7; y >= 0; y--) if (y !== 6) e = DC.p(m.get(8, y), e);
		const d = m.h;
		for (let y = d - 1; y >= d - 7; y--) f = DC.p(m.get(8, y), f);
		for (let x = d - 8; x < d; x++) f = DC.p(m.get(x, 8), f);
		let g = Infinity,
			h = null;
		for (const a of DC.F) {
			const b = a.bits,
...
class LC {
	...
	static rFP(a, b, c) {
		...
		return { bottomLeft: m, topLeft: l, topRight: n };
	}
	static cD(a, b, c, m) {
		const e =
			(LC.s(LC.cB(a, c, m, 5)) / 7 +
				LC.s(LC.cB(a, b, m, 5)) / 7 +
				LC.s(LC.cB(c, a, m, 5)) / 7 +
				LC.s(LC.cB(b, a, m, 5)) / 7) /
			4;
				...
				const oP = f
					...
					.map((p) => {
						return {
							x: p.x,
							y: p.y,
							score: p.score + M.pow(p.size - t.size, 2) / t.size,
							size: p.size,
						};
					})
					.sort((a, b) => a.score - b.score);
