...
class BM {
	...
	sR(l, t, w, h, v) {
		for (let y = t; y < t + h; y++) for (let x = l; x < l + w; x++) this.set(x, y, !!v);
	}
}
/* 1 GenericGF*/
class GF {
...
class GP {
	...
	a(o) {
		...
		const sd = W.u8ca(lc.length),
			ld = lc.length - sc.length;
...
class RS {
	...
	static dec(b, s2) {
		...
		if (!e) return oB;
		const sy = new GP(f, sC),
			sO = RS.rEA(f, f.b(s2, 1), sy, s2);
		if (sO === null) return null;
		const eL = RS.fEL(f, sO[0]);
		if (eL == null) return null;
		const eM = RS.fEM(f, sO[1], eL);
		for (let i = 0; i < eL.length; i++) {
			const po = oB.length - 1 - f.l(eL[i]);
			if (po < 0) return null;
	...
	static bz(d, w, h, r) {
		...
		for (let vR = 0; vR < vRC; vR++) {
			for (let hR = 0; hR < hRC; hR++) {
				...
				for (let y = 0; y < RS; y++)
					for (let x = 0; x < RS; x++) {
						let pL = gP.get(hR * RS + x, vR * RS + y);
						m += pL;
						min = M.min(min, pL);
						max = M.max(max, pL);
					}
				let avg = m / M.pow(RS, 2);
				if (max - min <= BZ.M) {
					avg = min / 2;
					if (vR > 0 && hR > 0) {
						const aNBP = (bP.get(hR, vR - 1) + 2 * bP.get(hR - 1, vR) + bP.get(hR - 1, vR - 1)) / 4;
		...
		for (let vR = 0; vR < vRC; vR++)
			for (let hR = 0; hR < hRC; hR++) {
				const l = BZ.nb(hR, 2, hRC - 3),
					t = BZ.nb(vR, 2, vRC - 3);
				let m = 0;
				for (let xR = -2; xR <= 2; xR++) for (let yR = -2; yR <= 2; yR++) m += bP.get(l + xR, t + yR);
				const th = m / 25;
...
class BS {
	...
	r(nB) {
		const z = this;
		if (nB < 1 || nB > 32 || nB > z.a()) W.e(`Cannot read ${nB.toString()} bits`);
		let r = 0;
		if (z.bio > 0) {
			const bL = 8 - z.bio,
				tR = nB < bL ? nB : bL,
...
class DC {
		...
		for (const x of v.alignmentPatternCenters)
			for (const y of v.alignmentPatternCenters)
				if (!((x === 6 && y === 6) || (x === 6 && y === d - 7) || (x === d - 7 && y === 6)))
					m.sR(x - 2, y - 2, 5, 5, true);
		m.sR(6, 9, 1, d - 17, true);
		m.sR(9, 6, d - 17, 1, true);
		if (v.versionNumber > 6) {
			m.sR(d - 11, 0, 3, 6, true);
			m.sR(0, d - 11, 6, 3, true);
		}
		return m;
	}
	static c(m, v, f) {
		const M = DC.M[f.dataMask],
			d = m.height,
			fPM = DC.m(v),
			c = [];
		let cb = 0,
	...
	static v(m) {
		const d = m.height,
			pV = Math.floor((d - 17) / 4);
		if (pV <= 6) return W.VERSIONS[pV - 1];
		let tR = 0,
			bL = 0;
	...
	static f(m) {
		let tL = 0,
			tR = 0;
		for (let x = 0; x <= 8; x++) if (x !== 6) tL = DC.p(m.get(x, 8), tL);
		for (let y = 7; y >= 0; y--) if (y !== 6) tL = DC.p(m.get(8, y), tL);
		const d = m.height;
		for (let y = d - 1; y >= d - 7; y--) tR = DC.p(m.get(8, y), tR);
		for (let x = d - 8; x < d; x++) tR = DC.p(m.get(x, 8), tR);
		let bD = Infinity,
			bFI = null;
		for (const a of DC.F) {
			const b = a.bits,
		...
		return bD <= 3 ? bFI : W.n('readFormatInformation bD:' + bD);
	}
	static b(c, v, e) {
		const ei = v.errorCorrectionLevels[e],
			db = [],
			f = ei.ecBlocks;
		let tc = 0;
		for (const b of f)
			for (let i = 0; i < b.numBlocks; i++) {
				...
				db.push({ nD, cw: [] });
				tc += nD + ei.ecCodewordsPerBlock;
			}
		if (c.length < tc) return W.n(`getDataBlocks codeWs.length < tc:${tc}/codeWs:${c.length}`);
		c = c.slice(0, tc);
		const s = f[0].dataCodewordsPerBlock;
		for (let i = 0; i < s; i++) for (const d of db) d.cw.push(c.shift());
		if (f.length > 1) {
			const sBC = f[0].numBlocks,
				lBC = f[1].numBlocks;
			for (let i = 0; i < lBC; i++) db[sBC + i].cw.push(c.shift());
		}
		while (c.length > 0) for (const d of db) d.cw.push(c.shift());
		return db;
	}
	static x(m) {
		const v = DC.v(m);
		...
		const c = DC.c(m, v, fI),
			d = DC.b(c, v, fI.errorCorrectionLevel);
		if (!d) return W.n('decodeMatrix d', d);
		const t = d.reduce((a, b) => a + b.nD, 0),
			r = W.u8ca(t);
		let ri = 0;
		for (const b of d) {
			const cb = RS.dec(b.cw, b.cw.length - b.nD);
	...
	static e(i, l) {
		...
		for (let y = 0; y < l.dim; y++)
			for (let x = 0; x < l.dim; x++) {
				...
				m.set(x, y, i.get(Math.floor(p.x), Math.floor(p.y)));
			}
		return {
			matrix: m,
			mF,
...
class LC {
	...
	static s = (vals) => vals.reduce((a, b) => a + b);
	static rFP(p1, p2, p3) {
		const d12 = LC.d(p1, p2),
			d23 = LC.d(p2, p3),
			d13 = LC.d(p1, p3),
	...
	static cD(tL, tR, bL, m) {
		const mS =
			(LC.s(LC.cB(tL, bL, m, 5)) / 7 +
				LC.s(LC.cB(tL, tR, m, 5)) / 7 +
				LC.s(LC.cB(bL, tL, m, 5)) / 7 +
				LC.s(LC.cB(tR, tL, m, 5)) / 7) /
			4;
		if (mS < 1) W.e('Invalid module size');
		const tD = Math.round(LC.d(tL, tR) / mS),
			sD = Math.round(LC.d(tL, bL) / mS),
			d = Math.floor((tD + sD) / 2) + 7;
		return { dim: d + (!(d % 4) ? 1 : d % 4 === 2 ? -1 : 0), moduleSize: mS };
	}
	static cBT(o, e, m, l) {
		const M = Math,
			sp = [{ x: M.floor(o.x), y: M.floor(o.y) }],
			st = M.abs(e.y - o.y) > M.abs(e.x - o.x),
		...
		const ds = [];
		for (let i = 0; i < l; i++) ds.push(sp[i] && sp[i + 1] ? LC.d(sp[i], sp[i + 1]) : 0);
		return ds;
	}
	static cB(o, e, m, l) {
		const rise = e.y - o.y,
			run = e.x - o.x,
			tE = LC.cBT(o, e, m, Math.ceil(l / 2)),
			aE = LC.cBT(o, { x: o.x - run, y: o.y - rise }, m, Math.ceil(l / 2)),
			mV = tE.shift() + aE.shift() - 1;
		let i;
		return (i = aE.concat(mV)).concat.apply(i, tE);
	}
	static sB(s, r) {
		const aS = LC.s(s) / LC.s(r);
		let e = 0;
		r.forEach((ratio, i) => {
			e += Math.pow(s[i] - ratio * aS, 2);
		});
		return { avgS: aS, err: e };
	}
	static sP(p, r, m) {
		try {
			const hR = LC.cB(p, { x: -1, y: p.y }, m, r.length),
				vR = LC.cB(p, { x: p.x, y: -1 }, m, r.length),
				M = Math,
				tLP = {
					x: M.max(0, p.x - p.y) - 1,
					y: M.max(0, p.y - p.x) - 1,
				},
				tL = LC.cB(p, tLP, m, r.length),
				bLR = {
					x: M.min(m.width, p.x + p.y) + 1,
					y: M.min(m.height, p.y + p.x) + 1,
				},
				bL = LC.cB(p, bLR, m, r.length),
				hE = LC.sB(hR, r),
				vE = LC.sB(vR, r),
				dD = LC.sB(tL, r),
				dU = LC.sB(bL, r),
				rE = M.sqrt(hE.err * hE.err + vE.err * vE.err + dD.err * dD.err + dU.err * dU.err),
				aS = (hE.avgS + vE.avgS + dD.avgS + dU.avgS) / 4,
				sE =
					(M.pow(hE.avgS - aS, 2) +
						M.pow(vE.avgS - aS, 2) +
						M.pow(dD.avgS - aS, 2) +
	...
	static l(m) {
		...
		const fP = [],
			...
			l1 = (y) => {
						...
						if (v === lb) l1++;
							...
							const aFPB = LC.s(ss) / 7,
								...
								vAP =
									...
									v;
							if (vFP) {
								const eX = x - ss[3] - ss[4],
									sX = eX - ss[2],
									l = { startX: sX, endX: eX, y: y },
									mQ = aF.filter(
										(q) =>
								...
								if (mQ.length > 0) mQ[0].bottom = l;
								else aF.push({ top: l, bottom: l });
							}
							if (vAP) {
								const eX = x - ss[4],
									sX = eX - ss[3],
									l = { startX: sX, y, endX: eX },
									mQ = aA.filter(
										(q) =>
											(sX >= q.bottom.startX && sX <= q.bottom.endX) ||
											(eX >= q.bottom.startX && sX <= q.bottom.endX) ||
				...
				W.cat(
					fP,
					aF.filter((q) => q.bottom.y !== y && q.bottom.y - q.top.y >= 2)
				);
				aF = aF.filter((q) => q.bottom.y === y);
				W.cat(
					aP,
					aA.filter((q) => q.bottom.y !== y)
				);
				aA = aA.filter((q) => q.bottom.y === y);
			};
		for (let y = 0; y <= m.height; y++) l1(y);
		W.cat(
			fP,
			aF.filter((q) => q.bottom.y - q.top.y >= 2)
		);
		W.cat(aP, aA);
		const fPG = fP
			...
			.map((q) => {
				...
				const l = [q.top.endX - q.top.startX, q.bottom.endX - q.bottom.startX, q.bottom.y - q.top.y + 1],
		...
		const mTR = LC.rL(m, tR),
			mTL = LC.rL(m, tL),
			mBL = LC.rL(m, bL),
			cAl = LC.fAP(m, aP, mTR, mTL, mBL);
		if (cAl)
			r.push({
				alignmentPattern: {
	...
	static fAP(m, a, tR, tL, bL) {
		...
		const bRFP = {
			...
			mBFP = (LC.d(tL, bL) + LC.d(tL, tR)) / 2 / s,
			c2TL = 1 - 3 / mBFP,
			eAP = {
		...
		let aps = a
			.map((q) => {
				...
				const ss = LC.sP({ x: M.floor(x), y: M.floor(y) }, [1, 1, 1], m);
				return { x, y, score: ss + LC.d({ x: x, y: y }, eAP) };
			})
			.filter((v) => !!v)
			.sort((a, b) => a.score - b.score);
		return { alignmentPattern: mBFP >= 15 && aps.length ? aps[0] : eAP, dim: d };
	}
}
/* 3 */
class JsQR {
		...
		for (const l of ls) {
			...
			if (dc)
				return {
					...
					version: dc.v,
					location: {
						topRightCorner: ex.mF(l.dim, 0),
						topLeftCorner: ex.mF(0, 0),
						bottomRightCorner: ex.mF(l.dim, l.dim),
						bottomLeftCorner: ex.mF(0, l.dim),
	...
	static async exec(data, width, height, providedOptions = {}) {
		...
		const i = o.inversionAttempts,
			...
			r = JsQR.s(t ? j.iv : j.bz);
		return !r && (i === 'attemptBoth' || i === 'invertFirst') ? JsQR.s(t ? j.bz : j.iv) : r;
	}
}
