...
class U {
	static isDeepEqual(a, b) {
		...
		if (Array.isArray(a)) {
			...
			for (let i = 0; i < a.length; i++) if (!U.isDeepEqual(a[i], b[i])) return false;
		} else if (at === 'object') {
			const ak = Object.keys(a);
			const bk = Object.keys(b);
			if (!U.isDeepEqual(ak, bk)) return false;
			for (const k of ak) if (!U.isDeepEqual(a[k], b[k])) return false;
		}
		return true;
	}
}
class V {
	...
	static compess(u8a) {
		...
		// console.log('compess u8a:', u8a);
		// console.log('compess n:', n);
		// console.log('compess l:' + l + '/s:' + s + '/b:' + b + '/0:' + n[l - 2] + '/1:' + n[l - 3]);
		...
		return new Promise((r) => {
			...
			i.onload = () => {
				...
				const d = x.getImageData(0, 0, s, s),
					...
					b = (n[l - 2] << 8) + n[l - 3],
					u = new Uint8Array(b);
				// console.log('decompress l:' + l + '/s:' + s + '/b:' + b + '/0:' + n[l - 2] + '/1:' + n[l - 3], n);
...
class F {
	static async shiftJISTableCompress(shiftJISTable, elm) {
		...
		const h = await Hasher.digest(str);
		console.log(`shiftJISTableCompress STEP 1:`, str);
		console.log(
			`shiftJISTableCompress STEP 2:${Object.keys(shiftJISTable).length}/${str.length}/${U.isDeepEqual(
				shiftJISTable,
				rT
			)}`,
			h
	...
	static compress(t) {
		...
		const u8a = new Uint8Array(k.length * 4);
		let c = 0;
		u8a.fill(0);
		for (const [k, v] of Object.entries(t)) {
			...
			u8a[c + 2] = v >> 8;
			u8a[c + 3] = v % 256;
			// console.log(`c:${c}/k:${k}/n:${n}/v:${v}/0:${u8a[c + 0]}/1:${u8a[c + 1]}/2:${u8a[c + 2]}/3:${u8a[c + 3]}`);
			c += 4;
		}
		return u8a;
	}
	...
	static async VersionCompress(Version, elm) {
		...
		console.log('VersionCompress STEP 0:' + str.length, Version);
		const rt = JSON.parse(str);
		F.vc(rt, keyMap, keys);
		console.log('VersionCompress STEP 1:', rt);
		const rj = JSON.stringify(rt);
		const rjq = F.ej(rj);
		const d = V.compess(B64U.s2U8a(rjq));
		console.log('VersionCompress STEP 2:' + rj.length + '/d:' + d.length, rjq);
		console.log('VersionCompress STEP 2a:', d);
		const u8aa = await V.decompress(d);
		const rjqa = B64U.ab2S(u8aa.buffer);
		console.log('VersionCompress STEP 3:', keyMap);
		const vMap = F.rm(keyMap);
		console.log('VersionCompress STEP 4:', vMap);
		console.log('VersionCompress STEP 4a:', JSON.stringify(vMap));
		const rjp = F.dj(rjqa, vMap);
		console.log('VersionCompress STEP 5:' + rj.length + '/rjp:' + rjp.length + '/' + (rj === rjp), rjp);
		const rtt = JSON.parse(rjp);
		F.vd(rtt, vMap);
	...
	static ej(json, p = F.p) {
		let a = json;
		for (const q of p) a = q[0] ? a.split(q[0]).join(q[1]) : a;
		return a;
	}
	static dj(j, vm) {
		let p = JSON.parse(JSON.stringify(F.p));
	...
	static vd(obj, vm) {
		if (Array.isArray(obj)) for (const r of obj) F.vd(r, vm);
		else if (typeof obj === 'object') {
			for (const k in obj) {
				const v = obj[k];
				delete obj[k];
